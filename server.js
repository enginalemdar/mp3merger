...
const PQueue = require('p-queue').default; // İstekleri sıraya almak için kütüphane
...
async function processAudioTask(req, res) {
    ...
    try {
        if (validPaths.length === 0) {
            ...
            const resampleFilterDefinitions = validPaths.map((_, i) => `[${i}:a]aresample=${targetSampleRate}[in${i}]`).join(','); // VIRGULLE AYRILIYOR

            // Sessizlik akışını tanımla (virgülle ayır)
            // aevalsrc -> [silence]
            const silenceFilterDefinition = `aevalsrc=0:s=${targetSampleRate}:d=${silenceDuration}[silence]`; // VIRGUL BURADAN SONRA GELECEK
            ...
            if (validPaths.length === 2) {
                ...
                // Hepsini tek zincirde virgul ile bağlıyoruz
                 filterComplex = `${resampleFilterDefinitions},${concatInputPads}concat=n=${totalConcatInputs}:v=0:a=1[a],[a]loudnorm=I=${targetLufs}:TP=-1.0:LRA=11[out]`;
                ...
                // Başlangıç filtrelerinin listesi (sessizlik + tüm resampleler), virgülle ayırılıyor
                 const initialFilters = `${silenceFilterDefinition},${resampleFilterDefinitions}`; // VIRGULLE AYIR
                ...
                // Filter complex string: initial filters, then concat, then loudnorm
                // Hepsini tek zincirde virgul ile bağlıyoruz
                 filterComplex = `${initialFilters},${concatInputPads}concat=n=${totalConcatInputs}:v=0:a=1[a],[a]loudnorm=I=${targetLufs}:TP=-1.0:LRA=11[out]`;
...
app.post('/merge', upload, async (req, res) => {
    ...
    const timestamp = Date.now(); // Her istek için yeni bir zaman damgası al
    try {
        ...
        console.log(`[${timestamp}] Merge isteği kuyrukta işlendi ve yanıt gönderildi.`);

    } catch (error) {
        // *** Hata Yakalama ve Yanıt Gönderme (Kuyruk ekleme sırasında bir hata olursa) ***
        // processAudioTask içindeki hatalar processAudioTask'ın kendi catch bloğunda yakalanır.
        // Bu blok, sadece kuyruğa ekleme sırasında veya çok nadir bir durumda processAudioTask'ın beklenmedik şekilde hata fırlatması durumunda tetiklenir.
        console.error(`[${timestamp}] Kuyruğa ekleme sırasında veya bekleme sırasında hata oluştu:`, error);
        if (!res.headersSent) {
             res.status(500).send(`İstek işlenirken veya kuyruğa eklenirken beklenmedik bir hata oluştu.`);
        } else {
             console.warn(`[${timestamp}] Kuyruk/bekleme hatası oluştu ancak yanıt zaten gönderilmişti. İstek ID: ${timestamp}`);
        }

    }
    // NOT: Geçici dosya temizliği artık processAudioTask fonksiyonunun kendi finally bloğunda yapılıyor.
    // Ana /merge handler'ının finally bloğuna gerek yok çünkü dosyalar processAudioTask içinde oluşturulup temizleniyor.
    // Ancak, Multer'ın oluşturduğu *ilk* geçici dosyaların temizliği, processAudioTask hata verdiğinde veya atlandığında sorun olabilir.
    // Daha sağlam bir yapı için Multer'ın dosyalarını da processAudioTask'ın finally bloğuna dahil etmek gerekiyor.
    // Mevcut kodunuzda filesToClean listesi zaten bu dosyaları içeriyor, bu yüzden processAudioTask içindeki finally bloğu yeterli.
    // Bu ana handler'da bir finally bloğu olması (ikinci kopyada olduğu gibi) gereksiz ve hatalara yol açabilir.
});


// Sunucuyu belirtilen portta dinlemeye başlayın
app.listen(port, () => {
